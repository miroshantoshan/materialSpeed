...
struct ContentView: View {
    ...
    var body: some View {
        ZStack(alignment: .bottom) {
            ...
            if showsHistory {
                ...
                HistoryPanel(model: model, close: closeHistory)
    ...
    private var metrics: some View {
        HStack(spacing: 12) {
            MetricCard(label: "Загрузка", value: model.download, unit: "Mbps")
            MetricCard(label: "Отдача", value: model.upload, unit: "Mbps")
    ...
    private var phaseLabel: String {
        ...
        case .finished: return "РЕЗУЛЬТАТ"
    ...
    private var phaseSubtitle: String {
        switch model.phase {
        case .idle: return "Нажмите кнопку, чтобы начать"
        case .ping: return "16 контрольных измерений"
        case .download: return "Измеряем устойчивую скорость загрузки"
        case .upload: return "Измеряем устойчивую скорость отдачи"
        case .finished: return model.showsSavedConfirmation ? "Результат сохранён в истории" : ""
        case .failed: return "Проверьте подключение и повторите"
        }
    }

    private var buttonTitle: String {
        if model.isRunning { return "Остановить" }
        return model.phase == .finished ? "Проверить снова" : "Начать тест"
    }
        ...
        VStack(spacing: 0) {
            ...
            if model.history.isEmpty {
                ...
                VStack(spacing: 10) {
                    ...
                    Text("Завершённые тесты появятся здесь")
                        .font(.system(size: 12))
                        .foregroundStyle(M3.muted)
                }
                Spacer()
...
private struct HistoryRow: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            HStack(spacing: 22) {
                HistoryMetric(label: "Загрузка", value: result.download, unit: "Mbps")
                HistoryMetric(label: "Отдача", value: result.upload, unit: "Mbps")
